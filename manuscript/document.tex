...
information bits at the two RRHs during the HARQ process. We assume a HARQ
protocol based on Chase Combining (CC) and a maximum number of $M$
retransmissions.
For the $m$-th (re)transmission, every consecutive $\log_2Q$ bits are encoded
into a label $p=0,\ldots, Q-1$ and then mapped to two constellation symbols
via a vector mapping function $\bm{\psi}^{(m)}[p] = [\psi_1^{(m)}[p],
\psi_2^{(m)}[p]]^T$, where $\psi_a^{(m)}[p]\in \mathcal{C}$ and $\mathcal{C}$ is
the set of constellation symbols (e.g. QAP, 16-QAM). The two symbols are  then
transmitted at the two RRHs, respectively. The received signal at the
$m$-th (re)transmission of the bit sequence corresponding to label $p$ is:
\begin{align}
...
exchanging the mappings of exactly 2 labels whenever a reduction in the
...
perturbation step by exchanging the mappings of $k_p$ labels, where integer
