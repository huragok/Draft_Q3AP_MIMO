...
\begin{abstract}
  ...
  and optimization of MoDiv for HARQ in a MIMO scenario under Rician fading
  channel to mitigate packet loss.
    % (how they do)
    % (how we do)
...
\begin{align}
    \mathbf{y}^{(m)} = \mathbf{H}^{(m)}\mathbf{P} \bm{\psi}_m[p] +
    \mathbf{n}^{(m)},
\end{align}
where $\mathbf{P}$ is a $N_T$-by-$N_T$ linear precoder. We assume that the
additive noise $\mathbf{n}^{(m)}\sim\mathcal{CN}(\mathbf{0}, \sigma^2\mathbf{I})$. Moreover,
...
\begin{align}
    \mathbf{H}^{(n)} = \sqrt{\frac{K}{K+1}}\mathbf{H}_0 +
    \sqrt{\frac{1}{K+1}} \mathbf{R}^{1/2}\mathbf{H}_w^{(n)}\mathbf{T}^{1/2}
\end{align}
...
\begin{align}
    ...
    \mathbf{H}^{(n)}\mathbf{P}\bm{\psi}_n[p]\|^2,
\end{align}
...
\begin{align}
    ...
    \mathbf{P}\mathbf{e}_n[p,q]\|^2} {2\sigma^2}}\right)\right]
    \label{eq:PEP_M}
\end{align}
...
\begin{align}
    ...
    \mathbf{P}\mathbf{e}_n[p,q]\|^2}{4\sigma^2}\right)\right]
    \label{eq:PEP_M_1}
\end{align}
...
\begin{proposition}
    ...
    \begin{subequations}
      \begin{align}
        \bm{\mu}_n[p,q] & = \sqrt{K/(K+1)}\mathbf{H}_0\mathbf{P}
        \mathbf{e}_n[p,q] \\
        \mathbf{S}_n[p,q] & =
        4\sigma^2\mathbf{I} +
...
\begin{proof}
    ...
    \end{align}
    where $\mathbf{C}_n[p,q] =
    (\mathbf{e}_n^H[p,q]\mathbf{P}^H\mathbf{T}\mathbf{P}\mathbf{e}_n[p,q])\mathbf{R}/
    (K+1)$. For multi-variate Gaussian variable $\mathbf{v}\sim\mathcal{CN}(\bm{\mu},
